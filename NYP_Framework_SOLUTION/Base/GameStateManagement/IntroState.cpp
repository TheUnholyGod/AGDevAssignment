...
void CIntroState::Update(double dt)
{
	if (Application::GetInstance().resolution[Application::GetInstance().current_res] != myRes)
	{
		...
		float halfWindowWidth = Application::GetInstance().GetWindowWidth() / 2.0f;
		float halfWindowHeight = Application::GetInstance().GetWindowHeight() / 2.0f;
		IntroStateBackground->SetPosition(Vector3(halfWindowWidth, halfWindowHeight, 0.0f));
		IntroStateBackground->SetScale(Application::GetInstance().resolution[Application::GetInstance().current_res]);
	}
